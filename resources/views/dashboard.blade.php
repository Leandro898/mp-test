<x-app-layout>
    ...
    <div class="p-6 bg-gray shadow-md rounded mb-4">
        @if(auth()->user()->mp_access_token)
            ...
            <form action="{{ route('mercadopago.unlink') }}" method="GET" onsubmit="return confirm('¿Estás seguro que deseas desvincular tu cuenta de Mercado Pago?')">
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-gray font-bold py-2 px-4 rounded">
        ...
        @else
            ...
            <form action="{{ route('mercadopago.connect') }}" method="GET">
                <button type="submit" class="bg-green-600 hover:bg-green-700 text-gray font-bold py-2 px-4 rounded">
